...
### ~~内置本地翻译模型（腾讯混元 MT 1.8B）~~ 已评估，不做（2026-08-19 用户拍板）；**2026-09-07 补注：MT2 版协议已转 Apache-2.0，见上方「翻译源相关」节，随 v0.5.0 重议**
...
- ~~目标进程中途退出~~ 已处理（d974054：退出后引擎仍送全零包无任何标志，改为每秒查存活，退出即切回全部声音并提示）；**中途开始播放**仍未测（选择器只列已出声的程序，所以这条只在「先选后停再放」时才会遇到）
