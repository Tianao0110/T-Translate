...
### 悬浮窗截图时的闪烁也许可以去掉（2026-08-19 起有条件了）

截图前把悬浮窗透明度降到 0 是目前保证不拍到自己的唯一机制，代价是自动刷新
每次都闪一下——挂着看字幕时尤其明显。此前 WDA_EXCLUDEFROMCAPTURE 从来没生效
过（句柄传错，已修），现在它真的能把窗口排除在截图之外了。

如果实测确认排除有效，`hideForCapture` 可以改成"仅在 affinity 返回 false 时
才降透明度"。**必须真机验证**：这东西跨 GPU/驱动不保证，判据是截出来的图里
有没有悬浮窗自己的内容——错了就是 OCR 自己的译文，比闪一下糟得多。

### 跨段术语一致性（2026-08-19 用户拍板：记账，下次做）

**文档独有的痛点**，划词和悬浮窗都不存在：文档是逐段翻译的，同一个术语在第 3 段
译成「张量」、第 40 段译成「张量积」、第 90 段又变回英文原词。用户一定遇到过，但
现在没有任何机制能发现——这比总结有用得多，因为总结用户可以自己读，术语漂移他
发现不了。

设想：一个 AI 动作扫描全文译文，把同一术语的不同译法列出来，一键写进**已有的
术语库**（翻译后自动替换 + 撤销那套）并重译受影响的段落。等于把两个既有功能接上。

⚠️ 它不是"加一条配置"就能完事的：现有框架的动作都是"对着一段内容跑一次"，这个
要跨段扫描 + 回写术语库 + 触发重译，是一种新的动作形态。
