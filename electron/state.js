...
//
// 数据流约定:
//   electron-store 是跨进程共享配置的单一数据源。
//   渲染进程通过 IPC (store-get/store-set) 读写 electron-store。
//   渲染进程的 Zustand stores (config.js, translation-store.js) 管理 UI 状态,
//   并在关键配置变化时同步到 electron-store (如语言、主题)。
//   主进程只从 electron-store 读取，不使用 executeJavaScript 注入渲染进程。
