...
| **截图 OCR**     | 截屏识别文字，56 种识别语言，7 个 OCR 引擎自动降级                                   |
...
截取屏幕区域进行文字识别，引擎不可用时自动降级到下一个，使用无感。56 种识别语言：中/繁/英/日与 37 种拉丁语言内置，韩文、西里尔、天城文、阿拉伯字母各需下载一个语言包，一个包覆盖同一文字的多种语言。本地引擎读不出内容时会自动改用其他引擎。
...
配了视觉模型（LM Studio / Ollama 里的 Qwen-VL、LLaVA 等）时，模型直接读截图本身，跳过「识别文字再理解」两跳，版面和图文混排都保得住；模型看不了图会自动退回文字路径。总结会作为附属挂在翻译历史条目下，可以单独删除。
...
配了大模型翻译源时，每个段落右上角可以让 AI **讲解这一段**（读的是原文，没翻译的段落也能用），结果展开在译文下方，再点一下收起。攒够两段后工具栏出现「整理笔记」，把已讲解的段落汇总成一份连贯笔记——它只覆盖你点开过的段落，面板上会写明这一点，不是全文摘要。
