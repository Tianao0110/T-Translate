...
配了视觉模型（LM Studio / Ollama 里的 Qwen-VL、LLaVA 等）时，模型直接读截图本身，跳过「识别文字再理解」两跳，版面和图文混排都保得住；模型看不了图会自动退回文字路径。总结挂在翻译历史条目下，可单独删除；理解模式的结果单独入史，重讲同一段会替换。
...
配了大模型翻译源时，每个段落右上角可以让 AI **讲解这一段**（读的是原文，没翻译的段落也能用），结果展开在译文下方，再点一下收起。攒够两段后工具栏出现「整理笔记」，把已讲解的段落汇总成一份连贯笔记——它只覆盖你点开过的段落，面板上会写明这一点，不是全文摘要。讲解笔记随进度一起保存，重开文件可恢复。

翻译完成后，**术语检查**找出译文里漏翻的术语，一键替换；替换处高亮，可单独撤销。
