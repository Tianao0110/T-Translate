...
**声音来源**：默认听全部声音，也可以只听某一个程序（Windows 11）。列表只显示已经发过声音的程序——没开过音频流的进程在系统层面根本不存在。浏览器所有标签页共用同一个音频进程，所以粒度到「程序」为止，做不到按网站区分。Windows 10 只能听全部声音。

字幕可导出 SRT（原文 + 译文两行）。无痕模式下听译照常可用，只是不写会话日志。

**模型需要下载**：设置 → 音频 → 听，基础模型 153MB（必需）、草稿引擎 168MB（可选）。会话运行时约占 600–700MB 内存，停止即释放。
