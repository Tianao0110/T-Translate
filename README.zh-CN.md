...
| **听译**         | 实时转写电脑正在播放的声音并逐句翻译，中/英/日/韩/粤五语，识别在本机，音频不落盘；可只听指定程序；可导出 SRT |
...
悬浮窗工具栏的声波图标切到听译，点 ▶ 开始：实时转写电脑正在播放的声音，逐句翻译成目标语言。看没有字幕的视频、听会议、追生肉都用得上。

识别在本机完成，**音频只在内存里过一遍，不写磁盘**，也不上传。译文走你配置的翻译源（选本地模型就是全程不出本机）。

支持中文、英语、日语、韩语、粤语，可手动指定或交给自动判定。装上可选的草稿引擎后，中英会边说边出字（不到一秒出首字），说完自动修正为定稿；不装则整句说完再出字。

**声音来源**：默认听全部声音，也可以只听某一个程序，或听全部但排除某一个（Windows 11）。列表只显示已经发过声音的程序——没开过音频流的进程在系统层面根本不存在。浏览器所有标签页共用同一个音频进程，所以粒度到「程序」为止，做不到按网站区分。Windows 10 只能听全部声音。

字幕可导出 SRT（原文 + 译文两行）。无痕模式下听译不可用。

**模型需要下载**：设置 → 听译模型，基础模型 153MB（必需）、草稿引擎 168MB（可选）。会话运行时约占 600–700MB 内存，停止即释放。

### AI 功能（讲解 / 总结）

在翻译之外多一层「读懂」，只有两个概念：**「讲解」帮你看懂这一段，「总结」帮你读完一长篇**。总结读长段给要点，划词窗、主面板、悬浮窗都能用；悬浮窗左上角的**讲解模式**打开后，截图不再翻译，直接讲清内容在说什么。配了视觉模型时由模型直接读图，看不了图自动退回文字路径。

总结挂在翻译历史条目下，理解结果单独入史。设置页可导入自定义动作。**需要大模型翻译源**，只会翻译的源用不了，设置页有标记。

### 语言选择

134 种语言，覆盖谷歌翻译支持的全部范围。选择器常用语言置顶、其余按字母分组，索引条可点可滑，排序跟随界面语言。列表里没有的语言可在底部自行添加，并可单独指定发给模型的名字。
