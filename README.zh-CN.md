...
| **文档翻译**     | PDF / DOCX / EPUB / TXT / SRT 等 9 种格式，逐段翻译，进度与讲解笔记可恢复；术语检查一键补上漏译术语 |
...
截取屏幕区域进行文字识别，引擎不可用时自动降级到下一个，使用无感。59 种识别语言：中/繁/英/日与 37 种拉丁语言内置，韩文、西里尔、天城文、阿拉伯字母、泰米尔文、泰卢固文、卡纳达文各需下载一个语言包，一个包覆盖同一文字的多种语言。本地引擎读不出内容时会自动改用其他引擎。
...
在翻译之外多一层「读懂」。**总结**读完长段内容给要点，划词窗、主面板、悬浮窗都能用，结果在原地折叠展开；悬浮窗左上角的**理解模式**开关打开后，截图不再翻译，而是直接讲清楚内容在说什么——适合光翻译还是看不懂的技术文档、公式、代码。

配了视觉模型（LM Studio / Ollama 里的 Qwen-VL、LLaVA 等）时，模型直接读截图本身，跳过「识别文字再理解」两跳，版面和图文混排都保得住；模型看不了图会自动退回文字路径。总结会作为附属挂在翻译历史条目下，可以单独删除；理解模式的结果则作为独立条目进历史（卡片带「AI」徽章，原文与讲解都在），同一段重新讲解会替换旧条目。
...
配了大模型翻译源时，每个段落右上角可以让 AI **讲解这一段**（读的是原文，没翻译的段落也能用），结果展开在译文下方，再点一下收起。攒够两段后工具栏出现「整理笔记」，把已讲解的段落汇总成一份连贯笔记——它只覆盖你点开过的段落，面板上会写明这一点，不是全文摘要。讲解笔记随翻译进度一起保存，7 天内重开同一个文件可恢复。

翻译完成后，工具栏的**术语检查**会找出术语库里有、但译文里仍保留原文的词，确认后一键替换；替换处在段落里双向高亮，可查看原词或单独撤销。零模型调用，只按当前目标语言的术语核对。
