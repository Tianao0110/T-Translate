...
## 8. 语言判断（language-detect.js、language-id.js）

划词、悬浮窗与文档翻译的「内容已是目标语言」都问这里；历史与朗读的原文语言、AI 动作的「按原文语言输出」也用它。

- **按文字类型切块，按多数判**（2026-09-24 用户报告）：旧判断见到一个汉字就判中文，目标语言为中文时，夹了汉字的英文段落直接显示原文；中日韩以外一律判英文。现在把文本切成同一种文字的连续块，一个中日韩字和一个词各算一份。「已是目标语言」= 目标语言占多数，且其他语言的每一块都不超过人名、术语的大小（3 个词或 4 个字，用户拍板）；夹了一整句外文或外文占多数，就整段翻成目标语言。
- 假名定中日：整段全是假名，或与汉字、假名连写的假名占中日韩字的 5% 以上，才算日文。颜文字里孤零零的「ツ」不算——中文里夹一个 `¯\_(ツ)_/¯` 曾让整段被判成日文。
- **同一种字母的语言交给 ELD**（Efficient Language Detector，`eld/extrasmall`，Apache-2.0）：英法德西、俄乌保、阿波乌、印地与马拉地，只看文字类型分不开。选它是因为纯 JS、无依赖、仍在维护，短句准确率与重量级检测器同档（作者的基准：推文 99.3%、词对 84.9%、单词 67.1%）。
- **只在主进程加载一份，且只在需要时加载**：实测每个进程多占约 40 MB，而划词窗整个才 40 MB 左右，三扇窗各带一份不可接受。先按文字类型判一遍，只有目标语言与文本用同一种字母、且答案取决于这些块时才加载——目标语言为中日韩时永远不加载。首次加载 80–150 ms，之后每次约 0.03 ms。esbuild 把动态 import 打成延迟初始化，词库随翻译栈只解析、不执行；ELD 是 devDependency，不进安装包的 node_modules。
- **只采用 ELD 自己标为可靠的答案**，否则这些块算「判不准」，答案取决于它们时返回 `inTarget: null`。null 按「有得翻」处理（`core/text.js` 的 `resolveSameLanguageTarget`）：「显示原文」照常翻译，「自动互译」照常互译。判不准的多是单个词：这样既不会把法语单词当英文原样显示，也不会让开着自动互译的用户查英文单词时被翻回英文。
- ELD 的语言码与目录对不上的：`ku`（阿拉伯字母的库尔德语）是目录里的 `ckb`。ELD 不认识的语言（印尼语、斯瓦希里语等）永远不会被判成「已是目标语言」，也就是总会翻译；答出的语言不属于该块的文字时当作没答。
- 渲染端没有 ELD：`stack-client.detectLanguages` 经 `stack:detect-language` 问主进程，1000 条一批；主进程不答时退回本地按文字类型判（同一种字母的块按该文字的主要语言算）。文档翻译拆段时只处理文字类型能定的，剩下的段落在 `parseDocument` 末尾一次性问主进程。
