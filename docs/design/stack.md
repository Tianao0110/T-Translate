...
- Anthropic / Gemini：`max_tokens` / 截断的回答不能当完整的成功——会缓存并展示半截译文。Gemini 这一半直到 v0.5.2 才真的做了（此前只有这句话，代码里没有）：`finishReason === 'MAX_TOKENS'` 判失败，而且先于「空回答」判断——思考把输出上限吃光时正文是空的，报「被截断」比报「无翻译结果」有用。Gemini 的安全阈值全部放开：翻译合法地要处理新闻、小说等类别，默认阈值拦太多；多段回答要拼接；状态行读的是 `message` 键不是 `error`。Anthropic 的 dangerous-direct 头在浏览器外是惰性的，保留只为请求形状稳定。
