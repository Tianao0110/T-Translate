...
1. 选新 build 号，跑 `node scripts/fetch/fetch-llama-runtime.js --pin bNNNN`：下载官方 Vulkan zip、与 GitHub 发布的 digest 比对、重写 `llama-manifest.json`、抽出 DLL；`tests/unit/tengine/llama-manifest.test.js` 守住装载顺序里的文件不被漏掉。
...
- 开关：`settings.llm.allowUnlistedModels`（关于页开发者区，默认关；或环境变量 `TT_TENGINE_DEV=1`）。打开后模型目录里任何 GGUF 都能出现在候选列表，带「未验证」标记。
...
- 记文本是单独一个开关 `settings.llm.trialLogText`（默认关，只对未验证模型生效，无痕模式下无视开关一律不写）：打开后试用日志里带提示词与输出，用来判断质量。正式模型永远没有这个开关。
...
| 引擎 | 样本 | CPU | GPU |
| --- | --- | --- | --- |
| OCR（ppocr，2026-09-07，RTX 4090 Laptop） | 整屏截图 | 1.95 s | 0.15 s |
...
- 连崩退避：两分钟内三次崩溃 → 一分钟内不再重生，坏掉的原生库不会被紧循环反复拉起。只对选择加入的宿主生效；音频宿主的重启由会话自己决定（一次性重启），不走这里。
- 被我们 kill 但还没退出的进程记为「已丢弃」：它的 exit 仍以 expected 报出去让适配器收尾，但不再算活宿主；替换进程已起来之后才到的旧 exit 一律忽略。
- ready 之前就死：等 `spawn()` 的调用方必须收到崩溃，而不是干等 ready 超时。

### 音频适配器（engines/audio.js）

- 这里接管的是原来 `listen/audio-engine-manager.js` 自己做的引擎状态工作：模型载入计时、「这个进程到过 asr-ready 没有」、重启决策需要的退出分类、provider 与 sherpa 只写在 stderr 上的回退标记、语音自检。会话语义留在 manager。
- `STDERR_SETTLE`：sherpa 的回退说明走 stderr，与 IPC 回复不是一条管道，health 读它之前要等一拍让它落地。
- 换后端不重启：worker 丢掉已载入的语音，下个请求在新后端重建，会话不中断。

### llama 运行时（runtime/）

- koffi 的 struct 与回调类型是进程全局的、不能重定义，同一进程第二次 `loadRuntime` 复用已注册的类型。注册的回调必须活过所有可能调用它们的原生调用，所以保留到进程结束。
- 符号先按声明的 DLL 绑定，找不到再翻别的：换版时搬了家的符号仍能找到，ABI 单测会点名它去了哪。
- 装载探针对整套 DLL 做哈希约 150 ms，每次宿主启动一次。
- 思考开启符的判定要**扫词表文本**，不能只看 control 属性：Qwen3 与 Hy-MT2 都把 `<think>` 当普通 token。
- 取消后上下文仍恰好持有已解码的 token（llama 会回滚被中止的 batch），前缀跟踪只在真正的解码错误时作废。
- 设备选择：`gpu` 指「一块显卡」——显存最大的独显，没有独显才用核显；从不用 llama 默认的「层摊到所有设备」（双卡笔记本会把一半模型放到核显上）。
