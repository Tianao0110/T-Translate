const zh = {
  ...
  privacy: {
    ...
    modules: {
      ...
      updates: { name: "检查更新", standard: "可用", secure: "可用", offline: "不联网", offlineShort: "不联网" }
    },
  ...
  about: {
    ...
    storage: {
      ...
      modelsDir: "模型目录",
      inProgramDir: "程序目录",
      inUserDir: "用户目录（安装目录不可写）",
      legacyLabel: "旧目录",
      legacyFound: "还有 {{count}} 个模型包（{{size}}）",
      openFolder: "打开文件夹",
      moveButton: "移到程序目录",
      moving: "正在移动",
      moved: "已移到程序目录",
      moveFailed: "移动失败：{{error}}，旧目录里的模型保持原样",
      cleanButton: "清理旧数据",
      cleanConfirm: "删除 {{path}} 及其中全部内容？设置、历史记录和缓存已复制到新目录。",
      cleaning: "正在清理",
      cleaned: "旧目录已清理",
  ...
  llm: {
    ...
    packLabel: "{{name}}（{{role}}）",
    packLabelUnlisted: "{{name}}（未验证 · {{role}}）",
    unverified: "未验证",
    roleUnlistedHint: "未列入白名单的模型：能载入不等于每个功能都好用，请自行判断；它的请求另写试用日志。默认当通用模型用，只有文件名属于混元 MT 系列（hy-mt、hunyuan-mt）的才按「仅翻译」处理",
    installed: "已安装",
    notInstalled: "未安装",
    mismatch: "文件不符",
    mismatchHint: "文件名和大小对，内容与官方哈希不一致——重新下载官方文件",
    checking: "校验中",
    fileLine: "文件 {{file}} · {{size}} · {{license}}",
    howTo: "从下面的链接下载后，把文件放进模型文件夹即可；程序只加载哈希校验通过的文件，下载不经我们的服务器",
    linkOfficial: "官方下载",
    linkMirror: "国内镜像",
    fileLabel: "模型文件",
    openFolder: "打开文件夹",
    ...
    roleGeneral: "通用",
    roleMt: "仅翻译",
    roleGeneralHint: "翻译 + AI 动作（总结、讲解、改写）",
    roleMtHint: "只做翻译；选它时 AI 动作自动改走其他大模型源",
    packChanged: "已切换到 {{name}}",
    enabledHint: "在「翻译源」里启用并排在前面，翻译和 AI 动作就会用它；没放模型文件时自动跳到下一个源，不拖慢",
    run: {
      title: "运行",
      backend: "后端",
      gpu: "显卡（{{device}}）",
      ...
      state: "状态",
      loaded: "已载入 {{file}}",
      idle: "未载入（用时自动载入，闲置 5 分钟后卸载）",
      speed: "速度",
      speedValue: "{{n}} tok/s",
      speedUnknown: "尚未测量",
      slowHint: "低于 8 tok/s：建议在「关于」里开启显卡加速",
      selfTest: "自检",
      testing: "自检中…",
      testOk: "自检通过：{{n}} tok/s",
      testFail: "自检失败：{{reason}}",
    ...
    dev: {
      ...
      report: "试用报告",
      reportLine: "载入 {{loads}} · 请求 {{requests}} · 失败 {{failures}} · 停滞 {{stalls}} · 空输出 {{empty}} · 循环 {{loops}} · 思考泄漏 {{leaks}} · 平均 {{tps}} tok/s",
      reportEmpty: "还没有试用记录",
      logText: "试用日志记录提示词与输出文本",
      logTextHint: "只对未列入模型生效，无痕模式下不写；用来判断输出质量",
    },
  },
  ocr: {
    ...
    tengineVision: {
      name: "内置视觉模型（PaddleOCR-VL）",
      desc: "程序自带的视觉模型，本机运行；每行文字都带位置，复杂排版、表格、手写更稳。两个文件按链接自取，放进模型文件夹即可",
      notInstalled: "未安装",
      partial: "不完整",
      mismatch: "文件不符",
      notReady: "两个模型文件都放好并通过校验后才能启用",
      howTo: "下载下面两个文件，原文件名放进模型文件夹，然后点重新检测",
      linkModel: "主模型（官方）",
      linkEncoder: "图像编码器（官方）",
      mirrorModel: "主模型（镜像）",
    ...
    packs: {
      ...
      manifestOffline: "离线模式不联网，当前只列出已安装的语言包",
  ...
  listen: {
    ...
    packs: {
      ...
      manifestOffline: "离线模式不联网，当前只列出已安装的模型",
      downloaded: "模型安装完成",
  ...
  providerError: {
    ...
    tengineNoModel: "未安装内置模型文件，请到设置 → 本地模型 里放入模型",
